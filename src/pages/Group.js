...
//navigate back to groups page
 ...
 useEffect(() => {
  const fetchGroupMembers = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/group/groupmember/${id}`);
...
//delete groupMember or leave a group
  const deleteGroupMember = async () => {
    ...
    try {
      const response = await axios.delete(`${process.env.REACT_APP_API_URL}/group/groupmember/${id}`, 
      { data: 
        { 
          user_id: user.id
        }
  ...
  return (
    <div className="container">
      ...
      <img src={defaultImage} alt="Group Default" className="group-image" />
        ...
        <p>{isMember ? "You are a member of the group" : "You are not a member of the group"}</p>
      {isMember ? (
      ...
      <section className="members">
        ...
        <ul>
          {members.map((member) => (
            <li key={member.user_id}>{member.username}{member.role}
              {isOwner && (
              <button onClick={() => deleteGroupMember(member.user_id)}>Delete user from group</button>
            )}
            </li>
          ))}
        </ul>
